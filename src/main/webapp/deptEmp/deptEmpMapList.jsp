...
<%
	...
	if(word == null || word.equals("")) {
		...
		// null일때 목록
		listSql = "SELECT de.emp_no empNo, CONCAT(e.first_name, ' ', e.last_name) name, d.dept_name deptName, de.from_date fromDate, de.to_date toDate FROM dept_emp de INNER JOIN employees e ON de.emp_no = e.emp_no	INNER JOIN departments d ON de.dept_no = d.dept_no LIMIT ?,?";
		...
		*/
		// 검색일 때 페이지
		cntSql = "SELECT COUNT(*) FROM dept_emp de INNER JOIN employees e ON de.emp_no = e.emp_no INNER JOIN departments d ON de.dept_no = d.dept_no WHERE e.first_name LIKE ?";
		cntStmt = conn.prepareStatement(cntSql);
		...
		// 검색일 때 목록
		listSql = "SELECT de.emp_no empNo, CONCAT(e.first_name, ' ', e.last_name) name, d.dept_name deptName, de.from_date fromDate, de.to_date toDate FROM dept_emp de INNER JOIN employees e ON de.emp_no = e.emp_no INNER JOIN departments d ON de.dept_no = d.dept_no WHERE e.first_name LIKE ? LIMIT ?, ?;";
		listStmt = conn.prepareStatement(listSql);
			...
			<div class = "container">
				...
				if(word == null) {
					...
					<%
						if(currentPage > 1){
					%>
							<a href="<%=request.getContextPath()%>/deptEmp/deptEmpMapList.jsp?currentPage=<%=currentPage-1%>&word=<%=word%>">이전</a>
					<%
						}
					%>
					<%
						if(currentPage < lastPage){
					%>
							<a href="<%=request.getContextPath()%>/deptEmp/deptEmpMapList.jsp?currentPage=<%=currentPage+1%>&word=<%=word%>">다음</a>
					<%
						}
					%>
